...
class Vox_util(object):
    ...
    def unproject_image_to_mem(self, rgb_camB, pixB_T_camA, camB_T_camA, Z, Y, X, assert_cube=False, xyz_camA=None,
        ...
        # unproject the height map
        height_values = None
        if heights_camB is not None:
            heights_camB = heights_camB.unsqueeze(2)
            y_memA = torch.reshape(xyz_memA[:, :, 1], [B, Z, Y, X]).unsqueeze(1).long()
            height_values = F.grid_sample(heights_camB, xyz_pixB, align_corners=False)  # (B, Y, Z, Y, X)
            height_values = torch.gather(height_values, 1, y_memA)
            height_values = torch.reshape(height_values, (B, 1, Z, Y, X))
            values = values * height_values

        values = values * valid_mem
        return values, height_values
