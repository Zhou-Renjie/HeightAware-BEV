...
def depth_to_grayscale(depth_prob, invert=False):
    """
    :param depth_prob: B x D x H x W
    """
    depth_index = torch.argmax(depth_prob, dim=1)
    D = depth_prob.shape[1]
    depth_normalized = depth_index.float() / (D - 1)
    if invert:
        grayscale = depth_normalized
    else:
        grayscale = 1.0 - depth_normalized
    return torch.stack([grayscale] * 3, dim=1)
