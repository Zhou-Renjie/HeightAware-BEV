...
def img_transform(img, resize_dims, crop):
    ...
    mask = Image.new('L', resize_dims, 255)
    mask = mask.crop(crop)
    return img, mask
...
def clip_3d_box_with_near_plane(corners_3d, near_plane):
    ...
    # 1. Collect all points in front of the near plane
    for i, corner in enumerate(corners_3d):
        if corner[2] >= near_plane:
            visible_points.append(corner)

    # 2. Check intersection with near plane for each edge
    for edge in edges:
        p1, p2 = corners_3d[edge[0]], corners_3d[edge[1]]
        z1, z2 = p1[2], p2[2]
        
        # If one point is in front and one behind, calculate intersection
        if (z1 < near_plane and z2 >= near_plane) or (z1 >= near_plane and z2 < near_plane):
            ...
            intersection[2] = near_plane  # Ensure it's on the near plane
...
class NuscData(torch.utils.data.Dataset):
    def __init__(self, nusc, is_train, data_aug_conf, centroid=None, bounds=None, res_3d=None, nsweeps=1, seqlen=1, refcam_id=1, get_tids=False, temporal_aug=False, use_radar_filters=False, do_shuffle_cams=True, 
        ...
        self.supervise_height = supervise_height
        ...
        if data_ratio < 1.0:
            self.scenes.sort()
            self.scenes = self.scenes[:int(len(self.scenes)*data_ratio)]
    ...
    def get_image_data(self, rec, cams):
        ...
        for cam in cams:
            ...
            img, mask = img_transform(img, resize_dims, crop)
            imgs.append(totorch_img(img))
            pix_masks.append(totorch_img(mask))
        ...
        return (torch.stack(imgs), torch.stack(rots), torch.stack(trans),torch.stack(intrins), torch.stack(pix_masks))
    ...
    def get_foreground_area(self, imgs, pix_T_cams, cam_T_velo, lrtlist_, index=None):
        N_ = lrtlist_.shape[0]
        img_num, imgC, imgH, imgW = imgs.shape
        foreground_areas = torch.zeros((img_num, imgH//8, imgW//8), dtype=torch.float32)
        incomplete_objects_areas = torch.zeros((img_num, imgH//8, imgW//8), dtype=torch.float32)

        for cam_id in range(img_num):
            foreground_area = np.zeros((imgH, imgW), dtype=np.int32)
            incomplete_objects_area = np.zeros((imgH, imgW), dtype=np.int32)

            lrtlist_cam = utils.geom.apply_4x4_to_lrt(cam_T_velo[cam_id:cam_id+1].repeat(N_, 1, 1), lrtlist_).unsqueeze(0)
            corners_cam = utils.geom.get_xyzlist_from_lrtlist(lrtlist_cam).reshape(N_, 8, 3)

            for n in range(N_):
                # Clip 3D bounding box
                clipped_points = clip_3d_box_with_near_plane(corners_cam[n], 0.1)
                if len(clipped_points) == 0:
                    # The entire bounding box is behind the camera
                    continue

                # Project to image plane
                clipped_points = torch.stack(clipped_points, dim=0)
                clipped_points_pix = utils.geom.apply_4x4(pix_T_cams[cam_id], clipped_points.reshape(1, -1, 3)).squeeze(0)
                clipped_points_pix = clipped_points_pix[..., :2] / clipped_points_pix[..., 2:3]

                # Check fully visibility using bounding box
                is_fully_visible = True
                if len(clipped_points_pix) > 0:
                    min_x = torch.min(clipped_points_pix[:, 0])
                    max_x = torch.max(clipped_points_pix[:, 0])
                    min_y = torch.min(clipped_points_pix[:, 1])
                    max_y = torch.max(clipped_points_pix[:, 1])
                    if min_x < 0 or max_x >= imgW or min_y < 0 or max_y >= imgH:
                        is_fully_visible = False

                if clipped_points_pix.shape[0] >= 3:
                    hull = ConvexHull(clipped_points_pix.cpu().numpy())
                    hull_points = clipped_points_pix[hull.vertices].cpu().numpy().astype(np.int32)
                    cv2.fillPoly(foreground_area, [hull_points], 1)

                    if not is_fully_visible:
                        cv2.fillPoly(incomplete_objects_area, [hull_points], 1)

                # visualize
                # save_path = "./debug/202511092225"
                # os.makedirs(save_path, exist_ok=True)
                # img_np = imgs[cam_id].permute(1,2,0).cpu().numpy()
                # img_int8 = (img_np * 255).astype(np.uint8)
                # overlay = np.zeros_like(img_int8)
                # overlay[foreground_area == 1] = [0, 0, 255]
                # overlay[incomplete_objects_area == 1] = [0, 255, 0]
                # alpha = 0.3
                # blended = cv2.addWeighted(img_int8, 1 - alpha, overlay, alpha, 0)
                # cv2.imwrite(os.path.join(save_path, "foreground_area_%08d_%02d.png" % (index, cam_id)), blended)

            foreground_areas[cam_id] = torch.from_numpy(
                cv2.resize(foreground_area, (imgW//8, imgH//8), interpolation=cv2.INTER_NEAREST))
            incomplete_objects_areas[cam_id] = torch.from_numpy(
                cv2.resize(incomplete_objects_area, (imgW//8, imgH//8), interpolation=cv2.INTER_NEAREST))
        return foreground_areas, incomplete_objects_areas
...
class VizData(NuscData):
    ...
    def get_single_item(self, index, cams, refcam_id=None):
        ...
        imgs, rots, trans, intrins, pix_masks = self.get_image_data(rec, cams)
        ...
        pix_mask_ref = pix_masks[refcam_id].clone()
        pix_mask_0 = pix_masks[0].clone()
        pix_masks[0] = pix_mask_ref
        pix_masks[refcam_id] = pix_mask_0
        ...
        img_num, imgC, imgH, imgW = imgs.shape
        foreground_areas = torch.zeros((img_num, 1, imgH//8, imgW//8), dtype=torch.float32)
        incomplete_objects_areas = torch.zeros((img_num, 1, imgH//8, imgW//8), dtype=torch.float32)
        ...
        if N_ > 0:
            ...
            pix_T_cams = utils.geom.merge_intrinsics(*utils.geom.split_intrinsics(intrins)) # per-cam to pixel
            ...
            if self.supervise_height:
                foreground_areas, incomplete_objects_areas = self.get_foreground_area(imgs, pix_T_cams, cam_T_velo, lrtlist_, index)
                foreground_areas = foreground_areas.unsqueeze(1)
                incomplete_objects_areas = incomplete_objects_areas.unsqueeze(1)
        else:
        ...
        if self.get_tids:
            return imgs, rots, trans, intrins, lidar0_data, lidar0_extra, lidar_data, lidar_extra, lrtlist, vislist, tidlist_, scorelist, seg_bev, valid_bev, center_bev, offset_bev, size_bev, ry_bev, ycoord_bev, radar_data, egopose, pix_masks, foreground_areas, incomplete_objects_areas
        else:
            return imgs, rots, trans, intrins, lidar0_data, lidar0_extra, lidar_data, lidar_extra, lrtlist, vislist, scorelist, seg_bev, valid_bev, center_bev, offset_bev, size_bev, ry_bev, ycoord_bev, radar_data, egopose, pix_masks, foreground_areas, incomplete_objects_areas
    ...
    def __getitem__(self, index):
        ...
        all_egopose = []
        all_pix_masks = []
        all_foreground_areas = []
        all_incomplete_objects_areas = []
        for index_t in self.indices[index]:
            ...
            (
                ...
                lrtlist, vislist, tidlist, scorelist, seg_bev, valid_bev, center_bev, offset_bev, 
                size_bev, ry_bev, ycoord_bev, radar_data, egopose,
                pix_masks, foreground_areas, incomplete_objects_areas)= self.get_single_item(index_t, cams, refcam_id=refcam_id)
            ...
            all_pix_masks.append(pix_masks)
            if self.supervise_height:
                all_foreground_areas.append(foreground_areas)
                all_incomplete_objects_areas.append(incomplete_objects_areas)
        ...
        all_pix_masks = torch.stack(all_pix_masks)
        if self.supervise_height:
            all_foreground_areas = torch.stack(all_foreground_areas)
            all_incomplete_objects_areas = torch.stack(all_incomplete_objects_areas)
        ...
        return all_imgs, all_rots, all_trans, all_intrins, all_lidar0_data, all_lidar0_extra, all_lidar_data, all_lidar_extra, all_lrtlist, all_vislist, all_tidlist, all_scorelist, all_seg_bev, all_valid_bev, all_center_bev, all_offset_bev, all_radar_data, all_egopose, all_pix_masks, all_foreground_areas, all_incomplete_objects_areas
