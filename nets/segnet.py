...
class HeightBranch(nn.Module):
    def __init__(self, in_channels, out_channels, hidden_channels=128, num_blocks=2):
        super().__init__()
        self.conv1 = nn.Conv2d(in_channels, hidden_channels, kernel_size=3, padding=1, bias=False)
        self.bn1 = nn.BatchNorm2d(hidden_channels)
        self.gelu = nn.GELU()
        self.res_blocks = nn.Sequential(*[ResidualBlock(hidden_channels, hidden_channels) for _ in range(num_blocks)])
        self.conv2 = nn.Conv2d(hidden_channels, out_channels + 1, kernel_size=1, padding=0)
        self.softmax = nn.Softmax(dim=1)

    def forward(self, x):
        out = self.gelu(self.bn1(self.conv1(x)))
        out = self.res_blocks(out)
        out = self.softmax(self.conv2(out))
        return out
...
class Encoder_res101_height_aware(Encoder_res101):
    def __init__(self, C, H):
        super().__init__(C)
        self.H = H
        self.height_branch = HeightBranch(in_channels=512, hidden_channels=C, out_channels=self.H)

    def forward(self, x):
        x1 = self.backbone(x)
        x2 = self.layer3(x1)
        x = self.upsampling_layer(x2, x1)
        f = self.depth_layer(x) # feat batch of SimpleBEV
        h = self.height_branch(x).clone()  # height distribution
...
class Encoder_res50(nn.Module):
    ...
    def forward(self, x):
        ...
        x2 = self.layer3(x1)
        x = self.upsampling_layer(x2, x1)
        x = self.depth_layer(x) # feat batch of SimpleBEV

        return x

class Encoder_res50_height_aware(Encoder_res50):
    def __init__(self, C, H):
        super().__init__(C)
        self.H = H
        self.height_branch = HeightBranch(in_channels=512, hidden_channels=C, out_channels=self.H)
...
class Encoder_eff(nn.Module):
    def __init__(self, C, version='b4'):
        ...
        if self.downsample == 16:
            if self.version == 'b0':
                self.upsampling_in_channels = 320 + 112
            elif self.version == 'b4':
                self.upsampling_in_channels = 448 + 160
            self.upsampling_out_channels = 512
        elif self.downsample == 8:
            if self.version == 'b0':
                self.upsampling_in_channels = 112 + 40
            elif self.version == 'b4':
                self.upsampling_in_channels = 160 + 56
            self.upsampling_out_channels = 128
        else:
            raise ValueError(f'Downsample factor {self.downsample} not handled.')

        self.upsampling_layer = UpsamplingConcat(self.upsampling_in_channels, self.upsampling_out_channels)
        self.depth_layer = nn.Conv2d(self.upsampling_out_channels, self.C, kernel_size=1, padding=0)
...
class Encoder_eff_height_aware(Encoder_eff):
    def __init__(self, C, H, version='b4'):
        super().__init__(C, version)
        self.H = H
        self.height_branch = HeightBranch(in_channels=self.upsampling_out_channels, hidden_channels=self.C, out_channels=self.H)

    def forward(self, x):
        x = self.get_features(x)  # get feature vector
        f = self.depth_layer(x)  # feature and depth head
        h = self.height_branch(x).clone()  # height distribution
        return f, h
...
class Segnet(nn.Module):
    def __init__(self, Z, Y, X, vox_util=None, 
        ...
        self.use_height_aware = use_height_aware
        ...
        if not self.use_height_aware:
            ...
            ## Height-aware encoder
            if encoder_type == "res101":
                self.encoder = Encoder_res101_height_aware(feat2d_dim, self.Y)
            elif encoder_type == "res50":
                self.encoder = Encoder_res50_height_aware(feat2d_dim, self.Y)
            elif encoder_type == "effb0":
                self.encoder = Encoder_eff_height_aware(feat2d_dim, version='b0', H=self.Y)
            else:
                # effb4
                self.encoder = Encoder_eff_height_aware(feat2d_dim, version='b4', H=self.Y)
        ...
        if self.supervise_height:
            self.height_weight = nn.Parameter(torch.tensor(0.0), requires_grad=True)
    ...
    def forward(self, rgb_camXs, pix_T_cams, cam0_T_camXs, vox_util, rad_occ_mem0=None):
        ...
        if self.use_height_aware:
            feat_camXs_, height_dist_ = self.encoder(rgb_camXs_)
        else:
            feat_camXs_ = self.encoder(rgb_camXs_)
        if self.rand_flip:
            feat_camXs_[self.rgb_flip_index] = torch.flip(feat_camXs_[self.rgb_flip_index], [-1])
            if self.use_height_aware:
                height_dist_[self.rgb_flip_index] = torch.flip(height_dist_[self.rgb_flip_index], [-1])
        ...
        if self.xyz_camA is not None:
            ...
            xyz_camA = None
        feat_mems_, weights_mems_ = vox_util.unproject_image_to_mem(
            feat_camXs_,
            utils.basic.matmul2(featpix_T_cams_, camXs_T_cam0_),
            camXs_T_cam0_, Z, Y, X,
            xyz_camA=xyz_camA,
            heights_camB=height_dist_)
        ...
        if self.use_height_aware:
            weights_mems = __u(weights_mems_) # B, S, C, Z, Y, X
            mask_mems = (torch.abs(weights_mems) > 0).float()
            weights_mems = utils.basic.reduce_masked_mean(weights_mems, mask_mems, dim=1)
            height_dist = __u(height_dist_)
        ...
        return raw_e, feat_e, seg_e, center_e, offset_e, weights_mems, height_dist
