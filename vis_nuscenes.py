...
def run_model(loader, index, model, d, img_dir, device='cuda:0', sw=None):
    (
        imgs_all, rots_all, trans_all, intrins_all, pts0_all, extra0_all, 
        pts_all, extra_all, lrtlist_velo_all, vislist_all, tidlist_all, scorelist_all, 
        seg_bev_g_all, valid_bev_g_all, center_bev_g_all, offset_bev_g_all, radar_data_all, egopose_all,
        _, _, _)= d
    ...
    for t in range(T):
        ...
        _, feat_bev_e, seg_bev_e, center_bev_e, offset_bev_e, _, _ = model(
                rgb_camXs=rgb_camXs,
...
def main(
    ...
    model = Segnet(Z, Y, X, use_radar=use_radar, use_lidar=use_lidar, use_metaradar=use_metaradar, do_rgbcompress=do_rgbcompress, encoder_type=encoder_type, use_height_aware=use_height_aware, rand_flip=False)
